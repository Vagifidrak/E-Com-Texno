...
namespace eCommerce.Shared.Helpers
{
    public class CategoryHelpers
    {
        public static List<CategoryWithChildren> MakeCategoriesHierarchy(List<Category> allCategories)
        {
            ...
            {
                var categoryWithChildren = allCategories.Select(item => new CategoryWithChildren() { Category = item, Children = allCategories.Where(child => child.ParentCategoryID == item.ID).ToList() }).ToList();
